...
namespace Fugui.Core
{
    public class UIOverlay
    {
        ...
        public Action<UIOverlay> UI { get; private set; }
        ...
        private OverlayDragPosition _dragButtonPosition;
        ...
        public UIOverlay(string id, Vector2 size, Action<UIOverlay> ui, OverlayFlags flags = OverlayFlags.Default, OverlayDragPosition dragButtonPosition = OverlayDragPosition.Auto)
        {
            ...
            _collapsable = !flags.HasFlag(OverlayFlags.NoClose);
            // Set the draggable behavior based on the provided parameter
            _draggable = !flags.HasFlag(OverlayFlags.NoMove);
            // Set the draggable behavior based on the provided parameter
            _noBackground = flags.HasFlag(OverlayFlags.NoBackground);
            // Set the flag that allow Anchor Position to be changed
            _noEditAnchor = flags.HasFlag(OverlayFlags.NoEditAnchor);
        ...
        public bool AnchorWindowDefinition(UIWindowDefinition window, AnchorLocation anchor, Vector2 anchorOffset)
        {
        ...
        public bool AnchorWindow(UIWindow window)
        {
        ...
        public bool AnchorWindow(UIWindow window, AnchorLocation anchor, Vector2 anchorOffset)
        {
            ...
            if (_dragButtonPosition == OverlayDragPosition.Auto)
            {
                switch (anchor)
                {
                    case AnchorLocation.TopCenter:
                        _dragButtonPosition = OverlayDragPosition.Top;
                        break;
                    ...
                    case AnchorLocation.BottomLeft:
                        _dragButtonPosition = OverlayDragPosition.Left;
                        break;

                    case AnchorLocation.MiddleRight:
                    case AnchorLocation.BottomRight:
                    case AnchorLocation.TopRight:
                        _dragButtonPosition = OverlayDragPosition.Right;
                        break;

                    case AnchorLocation.BottomCenter:
                        _dragButtonPosition = OverlayDragPosition.Bottom;
                        break;
                }
            }
        ...
        internal void Draw()
        {
            ...
            if (!_collapsed)
            {
                ...
                {
                    case OverlayDragPosition.Top:
                        ImGui.SetCursorScreenPos(new Vector2(screenPos.x, screenPos.y + _retractButtonWidth));
                        break;

                    case OverlayDragPosition.Right:
                        ImGui.SetCursorScreenPos(screenPos);
                        break;

                    case OverlayDragPosition.Bottom:
                        ImGui.SetCursorScreenPos(new Vector2(screenPos.x, screenPos.y - _retractButtonWidth));
                        break;

                    default:
                    case OverlayDragPosition.Left:
