...
namespace Fugui.Core.DearImGui.Renderer
{
	...
	internal sealed class RendererProcedural : IRenderer
	{
		...
		public void RenderDrawLists(CommandBuffer cmd, DrawData drawData)
		{
		...
		private unsafe void UpdateBuffers(DrawData drawData)
		{
			...
			{
				drawArgCount += drawData.DrawLists[n].CmdBuffer.Length;
			}
			...
			for (int n = 0, nMax = drawData.CmdListsCount; n < nMax; ++n)
			{
				...
				for (int meshIndex = 0, iMax = drawList.CmdBuffer.Length; meshIndex < iMax; ++meshIndex)
				{
		...
		private void CreateDrawCommands(CommandBuffer cmd, DrawData drawData, Vector2 fbSize)
		{
