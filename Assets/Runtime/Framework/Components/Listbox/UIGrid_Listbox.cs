...
namespace Fugui.Framework
{
    public partial class UIGrid
    {
        ...
        protected override void _customListbox(string text, List<IListboxItem> items, Action<int> itemChange, Func<string> itemGetter, UIListboxStyle style)
        {
            if (!_gridCreated)
            {
                return;
        ...
        public override void Listbox(string text, string selectedItemText, Action callback, UIListboxStyle style, int height)
        {
            if (!_gridCreated)
            {
                return;
