...
namespace Fu.Framework
{
    public partial class FuGrid
    {
        ...
        protected override void _buttonsGroup<T>(string text, List<T> items, Action<int> callback, Func<string> itemGetter, float width, Vector2 padding, FuButtonsGroupFlags flags, FuButtonsGroupStyle style)
        {
            if (!_gridCreated)
            {
                return;
