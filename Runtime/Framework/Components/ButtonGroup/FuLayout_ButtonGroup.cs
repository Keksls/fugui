...
namespace Fu.Framework
{
    public partial class FuLayout
    {
        ...
        {
            ButtonsGroup<TEnum>(text, itemChange, itemGetter, width, FuThemeManager.FramePadding, flags, FuButtonsGroupStyle.Default);
        }
        ...
        {
            _buttonsGroup<T>(text, items, callback, itemGetter, width, FuThemeManager.FramePadding, flags, FuButtonsGroupStyle.Default);
        }
        ...
        protected virtual void _buttonsGroup<T>(string text, List<T> items, Action<int> callback, Func<string> itemGetter, float width, Vector2 padding, FuButtonsGroupFlags flags, FuButtonsGroupStyle style)
        {
            ...
            for (int i = 0; i < nbItems; i++)
            {
                ...
                if (this._customButton(Fugui.AddSpacesBeforeUppercase(items[i].ToString()), new Vector2(itemWidth, 0), padding, Vector2.zero, btnStyle, FuThemeManager.CurrentTheme.ButtonsGradientStrenght) && !LastItemDisabled)
                {
