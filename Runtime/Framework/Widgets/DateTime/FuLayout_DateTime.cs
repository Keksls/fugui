...
namespace Fu.Framework
{
    public partial class FuLayout
    {
        ...
        public bool DateTimePicker(string text, ref DateTime currentDate)
        {
            ...
            // ========================= CALENDAR HEADER
            // Last month
            if (_customButton("<##dtp", Vector2.zero, Fugui.Themes.FramePadding, Vector2.zero, FuButtonStyle.Default, Fugui.Themes.CurrentTheme.ButtonsGradientStrenght))
            {
                ...
                for (int j = 0; j < 7; j++)
                {
                    ...
                    if (i == 0 && j < dayOfWeek || dayCount >= daysInMonth)
                    {
                        ...
                        if (i == 0)
                        {
                            ...
                            _customButton((daysInLastMonth - (dayOfWeek - j) + 1) + "##lm", btnSize, Vector2.zero, Vector2.zero, FuButtonStyle.Transparent, Fugui.Themes.CurrentTheme.ButtonsGradientStrenght, false, 0.5f);
                        }
                        // lines of next month
                        else
                        {
                            ...
                            _customButton(((dayCount % daysInMonth) + 1) + "##nm", btnSize, Vector2.zero, Vector2.zero, FuButtonStyle.Transparent, Fugui.Themes.CurrentTheme.ButtonsGradientStrenght, false, 0.5f);
                            dayCount++;
                        }
                        FuButtonStyle.Default.Pop();
                        LastItemDisabled = lastItemDisabled;
                    ...
                    else
                    {
                        ...
                        if (_customButton((dayCount + 1).ToString(), btnSize, Vector2.zero, Vector2.zero, selected ? FuButtonStyle.Selected : FuButtonStyle.Transparent, Fugui.Themes.CurrentTheme.ButtonsGradientStrenght, false, 0.5f))
                        {
                            // Day button clicked
