...
{
    /// <summary>
    /// AddNode for the Fu nodal system. Generated automatically.
    /// </summary>
    public sealed class AddNode : FuNode
    {
        ...
        public override void Compute()
        {
            ...
            int outSize = Math.Max(SizeOf(aType), SizeOf(bType));
            string outType = OutVectorType(outSize);

            float[] a = ExpandTo(ReadToArray(this, "A", aType, 0f), outSize, 0f);
            float[] b = ExpandTo(ReadToArray(this, "B", bType, 0f), outSize, 0f);

            float[] r = new float[outSize];
            for (int i = 0; i < outSize; i++)
            {
                // Apply rule: operate component-wise on overlap; for excess components, pass-through the larger vector (here b since we expand both with their edge value).
                // Our ExpandTo already handles scalar promotion; using 'b' component ensures v2+v3 keeps v3.z as-is.
                float A = (i < SizeOf(aType)) ? a[i] : (i < SizeOf(bType) ? 0f : a[0]);
                float B = (i < SizeOf(bType)) ? b[i] : (i < SizeOf(aType) ? 0f : b[0]);
                r[i] = (i < Math.Min(SizeOf(aType), SizeOf(bType))) ? (A + B) : ((SizeOf(bType) > SizeOf(aType)) ? B : A + 0f);
            }
