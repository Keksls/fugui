...
{
    /// <summary>
    /// ClampNode for the Fu nodal system. Generated automatically.
    /// </summary>
    public sealed class ClampNode : FuNode
    {
        ...
        {
            string vType = GetPortType("A");
            string minType = GetPortType("B");
            string maxType = GetPortType("T");
            int outSize = Math.Max(SizeOf(vType), Math.Max(SizeOf(minType), SizeOf(maxType)));
            string outType = OutVectorType(outSize);

            float[] v = ExpandTo(ReadToArray(this, "A", vType, 0f), outSize, 0f);
            float[] mn = ExpandTo(ReadToArray(this, "B", minType, 0f), outSize, 0f);
            float[] mx = ExpandTo(ReadToArray(this, "T", maxType, 1f), outSize, 1f);

            float[] r = new float[outSize];
            for (int i = 0; i < outSize; i++)
                r[i] = Mathf.Clamp(v[i], mn[i], mx[i]);
...
FuNodalPort portT = new FuNodalPort
{
    Name = "T",
    Direction = FuNodalPortDirection.In,
    DataType = "core/float",
