...
{
    /// <summary>
    /// DistanceNode for the Fu nodal system. Generated automatically.
    /// </summary>
    public sealed class DistanceNode : FuNode
    {
        ...
        public override void Compute()
        {
            ...
            int s = Math.Max(SizeOf(aType), SizeOf(bType));
            float[] a = ExpandTo(ReadToArray(this, "A", aType, 0f), s, 0f);
            float[] b = ExpandTo(ReadToArray(this, "B", bType, 0f), s, 0f);
            float sum = 0f;
            for (int i = 0; i < s; i++) { float d = a[i]-b[i]; sum += d*d; }
            SetPortValue("Out", "core/float", Mathf.Sqrt(sum));
        }
