...
{
    /// <summary>
    /// DotNode for the Fu nodal system. Generated automatically.
    /// </summary>
    public sealed class DotNode : FuNode
    {
        public override string Title => "Dot";
        ...
        public override void Compute()
        {
            ...
            int outSize = Math.Max(2, Math.Min(SizeOf(aType), SizeOf(bType))); // require at least 2 components
            outSize = Math.Min(outSize, 4);
            float[] a = ExpandTo(ReadToArray(this, "A", aType, 0f), outSize, 0f);
            float[] b = ExpandTo(ReadToArray(this, "B", bType, 0f), outSize, 0f);
            float sum = 0f;
            for (int i = 0; i < outSize; i++) sum += a[i] * b[i];
            SetPortValue("Out", "core/float", sum);
        }
