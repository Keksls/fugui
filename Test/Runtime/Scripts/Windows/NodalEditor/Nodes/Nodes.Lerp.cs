...
{
    /// <summary>
    /// LerpNode for the Fu nodal system. Generated automatically.
    /// </summary>
    public sealed class LerpNode : FuNode
    {
        ...
        public override void Compute()
        {
            ...
            string tType = GetPortType("T");
            int outSize = Math.Max(SizeOf(aType), SizeOf(bType));
            outSize = Math.Max(outSize, SizeOf(tType));
            ...
            float[] t = ExpandTo(ReadToArray(this, "T", tType, 0f), outSize, 0f);

            float[] r = new float[outSize];
            for (int i = 0; i < outSize; i++)
            {
                r[i] = Mathf.Lerp(a[i], b[i], t[i]);
            }
