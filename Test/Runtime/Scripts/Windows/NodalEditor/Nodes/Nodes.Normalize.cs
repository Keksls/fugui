...
{
    /// <summary>
    /// NormalizeNode for the Fu nodal system. Generated automatically.
    /// </summary>
    public sealed class NormalizeNode : FuNode
    {
        ...
        public override void Compute()
        {
            ...
            float len = 0f;
            for (int i = 0; i < outSize; i++) len += a[i]*a[i];
            len = Mathf.Sqrt(Mathf.Max(1e-8f, len));
            float[] r = new float[outSize];
            for (int i = 0; i < outSize; i++) r[i] = a[i]/len;
